...
const EAS_ADDRESS =
  process.env.EAS_ADDRESS || "0x4200000000000000000000000000000000000021";

async function main() {
  if (!EAS_ADDRESS) {
    throw new Error(
      "EAS_ADDRESS environment variable is required to register the schema"
    );
  }
  ...
  const registryAddress = await eas.getSchemaRegistry();
  ...
  if (existing && existing.schema && existing.schema !== "") {
    ...
    console.log(
      `Schema registered in block ${receipt?.blockNumber ?? "unknown"}`
    );
  }
